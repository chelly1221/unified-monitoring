...
function DialogContent({
  ...
  // Workaround for Chrome fullscreen rendering bug - force reflow
  React.useEffect(() => {
    const el = overlayRef.current
    if (el) {
      // Force reflow by reading offsetHeight then modifying style
  ...
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "bg-background fixed top-[50%] left-[50%] z-[9999] grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border p-6 shadow-lg outline-none sm:max-w-lg",
          className
        )}
        style={style}
        {...props}
